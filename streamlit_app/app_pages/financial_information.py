...
def _process_stock_orders_submission(*, send_requested: bool) -> None:
    st.session_state["stock_orders_upload_result"] = None
    st.session_state["stock_orders_send_message"] = None

    uploaded_file = st.session_state.get("stock_orders_uploaded_file")
    if uploaded_file is None:
        st.error("Debes cargar un archivo CSV de órdenes antes de continuar.")
        return
    ...
    try:
        ...
        if send_requested:
            ...
            st.success("Archivo validado correctamente. Ahora ya está listo para envío.")
    except (BackendConfigurationError, ApiGatewayClientError) as exc:
        _render_processing_error(
            "El archivo es válido, pero no fue posible enviarlo al backend.",
        ...
        _render_processing_error(
            "No fue posible procesar el CSV de órdenes.",
...
def _render_uploaded_file_preview() -> None:
    uploaded_file = st.session_state.get("stock_orders_uploaded_file")
    if uploaded_file is None:
        return
    ...
    try:
        ...
        st.warning(
            "Se cargó un archivo, pero no fue posible mostrar la vista previa inicial. "
            "Puedes continuar con la validación para obtener el detalle técnico."
        )
        return

    st.subheader("Vista previa inicial")
    st.caption(
        "*Estas son las primeras 5 filas detectadas en el archivo cargado. Esta revisión rápida ayuda a confirmar "
...
st.write(
    "Carga el histórico operativo de movimientos de acciones para conservar el contexto transaccional real de compras y ventas. "
    "Este archivo es clave para reconciliar ejecución, volumen, comisiones y trazabilidad antes de persistir datos financieros."
)
st.caption(
    "*La carga de movimientos de acciones complementa los snapshots técnicos con evidencia operativa real. Permite medir ejecución, "
    "detectar duplicados, reconstruir decisiones históricas y preparar persistencia consistente hacia DynamoDB.*"
)
...
with st.form("stock_orders_upload_form", clear_on_submit=False, border=True):
    ...
    st.caption(
        ...
        "Internamente el archivo se renombra con el patrón `stock-order-timestamp-america-bogota-trii.csv`.*"
    )
    st.file_uploader(
        "Archivo CSV de órdenes",
        type="csv",
        key="stock_orders_uploaded_file",
        help="Carga un único archivo CSV con el histórico de órdenes de Trii.",
        width="stretch",
    )
...
if validate_requested or send_requested:
    _process_stock_orders_submission(send_requested=send_requested)

_render_uploaded_file_preview()
...
if upload_result is not None:
    ...
    metrics_columns[0].metric("Órdenes válidas", upload_result.record_count)
    metrics_columns[1].metric("Símbolos detectados", len(upload_result.symbols))
    ...
    with st.container(border=True):
        ...
        st.write("**Símbolos detectados:** " + ", ".join(upload_result.symbols))
